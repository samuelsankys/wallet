import { Entity } from '../../../service-transaction/src/application/entity';
...
export class Transaction extends Entity<TransactionProps> {
  ...
  public refund(transaction: Transaction, currentBalance: number): void {
    if (transaction.status !== TransactionStatusEnum.COMPLETED)
      throw Error('Is not a valid transaction.');
    if (transaction.type !== TransactionTypeEnum.Purchase)
      throw Error('Not is posible refunded this transaction.');
  ...
  public reversal(transaction: Transaction, currentBalance: number): void {
    ...
    if (transaction.type === TransactionTypeEnum.Reversal)
      throw Error('Not is posible reversal this transaction.');
